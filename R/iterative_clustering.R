...
iterative_clustering <- function(seurat_object, max_iterations = 10, min_score = 150, cluster_size = 20){
  ...
  for (i in 1:max_iterations-1){
    ...
    seurat_object <- clustering_iteration(seurat_object, min_score, cluster_size)
...
leiden_clustering <- function(seurat_object, num_clusters = 2, score_limit = 150, min_size = 20){
...
clustering_score <- function(seurat_object){
  ...
  require(Seurat)
  de.genes <- FindMarkers(seurat_object, ident.1 = 1, ident.2 = 2, logfc.threshold = 1, min.pct = 0.5, recorrect_umi = FALSE)
